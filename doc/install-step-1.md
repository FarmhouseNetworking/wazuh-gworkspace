...
At [https://console.cloud.google.com/](https://console.cloud.google.com/) create a new project called `Wazuh` then switch to that project.
...
* the [Google Workspace Alert Center API](https://console.cloud.google.com/apis/api/alertcenter.googleapis.com)

In the [API & Services menu](https://console.cloud.google.com/apis) select the [Credentials screen](https://console.cloud.google.com/apis/credentials) and create a **service account** with an **Service account name** of for example `Wazuh Monitoring`. Skip the optional steps and click Done. Click on the new service account and look for `Unique ID` and copy it down, you will need this for the `client id` later on.

Once the service account is created, select the "Keys" option from the horizontal menu, and create a new JSON key. Be sure to save the key in your password manager since you cannot download it afterwards.

# Create a Google Workspace service account
Now, in the Google Workspace admin allow the GCP service account [Domain-wide delegation](https://admin.google.com/ac/owl/domainwidedelegation) by entering `Unique ID` as the `client id`, and giving it two scopes:
...
* in **Admin roles and privileges**
  * create a custom role called `Wazuh Role` and select the following priviledges
    * in `Alert Center` check `View access`
    * check the `Reports` role
    * add new user as a member of the new role
